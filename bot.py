...
- mycinebd.com plain text (clickable link না)
...
# ── posted_movies.json — load & save ──────────────────────────────────────────
def load_posted_ids() -> set:
    if POSTED_FILE.exists():
        try:
            data = json.loads(POSTED_FILE.read_text())
            ids  = set(data.get("posted_ids", []))
            print(f"Loaded {len(ids)} posted IDs from {POSTED_FILE}")
            return ids
        except Exception as e:
            print(f"Could not read {POSTED_FILE}: {e}")
    return set()
...
def save_posted_ids(posted_ids: set):
    ...
    print(f"Saved {len(id_list)} IDs to {POSTED_FILE}")
...
def fetch_one_movie(posted_ids: set) -> dict | None:
    ...
    try:
        ...
        print(f"Movie detail fetch failed: {e}"); return None
...
SLOT_TEMPLATES = {"morning": MORNING_TEMPLATES, "noon": NOON_TEMPLATES, "evening": EVENING_TEMPLATES}
